...
X_noise = X.copy()
Y_noise = Y.copy()
bitsToFlip = np.arange(1,9)
for bitsFlip in bitsToFlip:
    percent = 0
    for i,y in zip(X_noise, Y_noise):
        x_test = i.copy()
        fliping = np.random.randint(155, size=(bitsFlip))
        for j in fliping:
            x_test[j] = 1 if x_test[j]  == 0 else 0  
        X = np.concatenate((X, x_test.reshape(156,1).T), axis=0)
        Y = np.concatenate((Y, y.reshape(7,1).T), axis=0)
...
print("Accuracy Score is {0}".format((count/(26*9))*100))
...
for bitsFlip in bitsToFlip:
    ...
    for i in range(1000):
        ...
        fliping = np.random.randint(155, size=(bitsFlip))
        for j in fliping:
            x_test[int((j/12) - 1)][(j%13) - 1] = 1 if x_test[int((j/12) - 1)][(j%13) - 1]  == 0 else 0 
        ...
        if v == checkLetter:
            percent+=1
    print("Accuracy when {0} percent bits flipped is {1}"
